...
# config
if os.path.exists(f'model_5fold/bert-{experiment_no}/config.pkl'):
    ...
    args['train_file_path'] = f'{args["data_path"]}/news.csv'
    args['train_news_path'] = None
    args['output_path'] = f'model_5fold/bert-{args["experiment_no"]}'
...
# save config
if not os.path.exists(f'model_5fold/bert-{experiment_no}/config.pkl'):
...
for fold, (train_index, test_index) in enumerate(kf.split(dataset)):
    ...
    # get training and testing data, dataloader
    ...
    for epoch in range(int(args['epochs'])):
        ...
        for step, train_batch in enumerate(epoch_iterator):
            train_batch = tuple(t.to(device) for t in train_batch[:-2])
            train_inputs = {
...
# calculate average loss on every epoch
average_train_loss = np.mean(np.array(all_train_loss), axis=0)
average_test_loss = np.mean(np.array(all_test_loss), axis=0)

num_of_epochs = np.argmin(average_test_loss, axis=0)+1

###########################################

# train a new model with all data
...
# save config
if not os.path.exists(f'model_5fold/bert-{experiment_no}/config.pkl'):
    with open(f'model_5fold/bert-{experiment_no}/config.pkl', 'wb') as f:
        pickle.dump(args, f)
...
for epoch in range(int(num_of_epochs)):
    print(f'Epoch: {epoch}')
    ...
    train_loss = evaluate(model, 'train', dataloader, epoch)
...
# init tensorboard
writer = SummaryWriter(f'tensorboard_5fold/bert-{experiment_no}')

# write 5-fold result
for epoch in range(int(args['epochs'])):
    writer.add_scalars(f'Loss/train', {f'{fold+1}-fold': all_train_loss[fold][epoch]
                                      for fold in range(int(args['kfold']))}, epoch)
    writer.add_scalars(f'Loss/test', {f'{fold+1}-fold': all_test_loss[fold][epoch]
                                      for fold in range(int(args['kfold']))}, epoch)

for epoch in range(int(num_of_epochs)):
    writer.add_scalar('Loss/final', final_train_loss[epoch], epoch)
